...
| `--model <MODEL>` | 埋め込みモデル (`bge-m3` / `gte` / `granite` / `distiluse` / `small` / `large` / `base` / `paraphrase` / `mpnet` / `paraphrase-q` / `e5-instruct`) | `bge-m3` |
...
| --- | --- | --- | --- |
| `onnx`(既定) | ONNX Runtime(`fastembed`) | 上表の `e5-instruct` 以外すべて | ネイティブの ONNX Runtime バイナリを取得・リンクする |
| `candle`(既定) | Candle(ピュア Rust) | `e5-instruct` ほか XLM-RoBERTa 系 safetensors | ONNX 変換が無いモデルも HF の safetensors から直接読み込む |
