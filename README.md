...
- **モデル選択**: 既定は用語集ベンチマーク(日英中混在)で誤統合が最も少なく適合率が最高の `gte-multilingual-base`。clusterF1/再現率を優先するなら `--model granite`、軽量・最速を優先するなら他モデルへ `--model` で切替可能(選択肢は下の[オプション](#オプション)表、詳細な比較は [`docs/benchmarks.md`](docs/benchmarks.md))
...
| `--model <MODEL>` | 埋め込みモデル (`gte` / `granite` / `distiluse` / `small` / `large` / `base` / `paraphrase` / `mpnet` / `paraphrase-q`) | `gte` |
...
| 値 | モデル | 次元 | 特徴 |
| --- | --- | ---: | --- |
| `gte` | gte-multilingual-base | 768 | **既定**。誤統合が最小で適合率最高。CJK に強い・約 1.2GB |
| `granite` | granite-embedding-278m-multilingual | 768 | clusterF1 最良(0.705)・日本語明示学習だが誤統合は gte より多め。約 1.1GB |
...
// 既定は gte-multilingual-base(誤統合最小・高適合率)。軽量・最速の E5 small へ切り替える例:
