...
# 素性能サマリ + 5 刻みスイープ
cargo run --example benchmark                  # 既定 (granite-278m-multilingual)
cargo run --example benchmark -- gte           # gte-multilingual-base(高適合率)
cargo run --example benchmark -- granite-278m  # granite-278m(既定・ユーザー定義)
...
- **既定は gte-multilingual-base**(誤統合の少なさを最優先)。真ピーク 0.682・ピーク時 P=0.782 で最高適合率、
  純粋分類 best-F1=0.776 も最良。閾値 70 のクラスタ誤統合 17 件・素ペア FP 2 件と最も少ない。ユーザー定義
  モデル(単一ファイル ONNX、約 1.2GB・`cos_baseline=0.42`)として読み込む。
- **clusterF1/再現率を最優先するなら granite-278m-multilingual**(IBM Granite Embedding R1・Apache 2.0)。
  clusterF1 真ピーク 0.705 で全モデル中最高、速度・サイズも gte 以上。日本語は明示的な学習対象。ただし
  閾値 70 の誤統合が 24 件(素ペア FP 19 件 = gte の約 10 倍)で「猫=犬」型のデータ破壊が増えるため**既定に
  はせず選択肢**(`--model granite`・`cos_baseline=0.44`)に留めた。表記ゆれ統合は誤統合が取りこぼしより
  痛いという方針の実例(clusterF1 が高くても誤統合が多ければ既定にしない)。
