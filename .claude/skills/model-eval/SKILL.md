...
| `openai-small` / `openai-large`(API・未計測) | OpenAI text-embedding-3-small / -large(重みダウンロード無しの API サービス) | OpenAI 利用規約(代表選出用トークナイザは Xenova/text-embedding-ada-002 の cl100k_base 変換・原典 tiktoken は MIT) |
...
# OpenAI API バックエンド(要 OPENAI_API_KEY・ネットワーク接続・API 利用料。HF_HUB_OFFLINE では
# 代表選出用トークナイザ Xenova/text-embedding-ada-002 のキャッシュが必要)
