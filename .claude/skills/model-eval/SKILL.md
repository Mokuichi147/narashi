...
- **既定は bge-m3**(精度と誤統合の少なさを両立)。clusterF1 真ピーク **0.725 で全モデル中最高**、かつ
  ピーク時 P=0.983・真ピーク時のクラスタ誤統合わずか 1 件と**最も誤統合も少ない**。「高 clusterF1 は誤統合増と
  引き換え」(granite-278m は 0.705 だが誤統合 24 件)というトレードオフを破り、精度・安全性・サイズ
  (1.06GB)で gte を上回る。`BAAI/bge-m3` の **fp16 単一 ONNX**(`Xenova/bge-m3`・`cos_baseline=0.072`)を
  ユーザー定義モデルとして読み込む。唯一の弱点は推論速度(gte の約 3 倍・≈16ms/語)。
...
- 評価対象外: jina-v3(CC-BY-NC)・Qwen3-Embedding(last-token プーリング非対応)・e5-large-instruct
  (fp16 単一版なし)・ruri-base(ONNX 未配布)。詳細は benchmarks の該当節を参照。
