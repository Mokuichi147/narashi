...
cargo run --example benchmark                  # 既定 (e5-small)
...
速度は `per_text`(1 用語あたり)で比較する。出力末尾の**閾値スイープ表**(50〜95 を 5 刻みの
分類F1 / クラスタF1)で、既定 70 以外の動作点も必ず確認する — モデルにより clusterF1 のピーク閾値が
大きく異なる(E5 系は ~70、paraphrase 系は ~85–90)。
...
- **clusterF1@70** が最重要 = `normalize` の実出力(推移閉包込み)と正解の一致度 = 実運用の挙動。
- **P@70 / R@70**: 既定閾値での適合率/再現率。表記ゆれ統合では**誤統合(低 Precision)はデータ破壊**で
  取りこぼし(低 Recall)より痛い。既定モデルは Precision を重視して選ぶ。
- **best-F1 / best-thr**: 閾値非依存の上限と、その最適閾値。best-thr が既定 70 から大きく外れるモデルは
  `model_spec().cos_baseline` の再校正で 70 に寄せられる(E5 系は 0.70、paraphrase 系は 0.30)。
- **margin**: `正例min − 負例max`。0 に近いほど正例・負例が分離している。

## 4. 反映

- 結果表と所見を **`docs/benchmarks.md` に追記/更新**(履歴として残す)。
...
`docs/benchmarks.md` 参照。要約:
- **既定の閾値 70 固定では e5-small** が最良(高 Precision・最速級)。
- **閾値を調整できるなら paraphrase-MiniLM @90** が clusterF1 ピーク 0.606 で全モデル中最高かつ最速級。
- e5-large は僅差で精度上位だが約 8 倍遅い。e5-base は劣後。量子化版は現環境で実行不可。
